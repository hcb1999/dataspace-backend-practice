...
*/
   ...
   * @param assetNo 
   */
  async createVcOrg(user: User, marketNo: number): Promise<any> {
  ...
  async createVcAsync(data: any): Promise<any> {

    console.log("market createVcAsync data : "+JSON.stringify(data));
    const marketNo = data.keyValue;
    ...
    try {
      ...
      const modifyMarket = {
        state: 'S2',
        vcId: `AL_DS_NFT-${marketNo}`,
        ...(vcType && { marketVcType: vcType }) // vcType이 있을 때만 객체에 병합
      };
