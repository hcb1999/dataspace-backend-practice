...
export class ProductService {
   ...
   */
  async create(user: User, files : any, createProductDto: CreateProductDto): Promise<any> {
    ...
    try {
      ...
      const advertiserInfo = await this.advertiserRepository.findOne({ where:{userNo} });
      if (!advertiserInfo) {
        const advertiserInfo1 = {userNo, advName:user.nickName};

        // console.log("===== creatorInfo : "+ creatorInfo);
        const newAdvertiser = queryRunner.manager.create(Advertiser, advertiserInfo1);
        const result = await queryRunner.manager.save<Advertiser>(newAdvertiser);
      }
   ...
   */
  async update(user: User, productNo: number, files : any, modifyProductDto: ModifyProductDto): Promise<void> {
    ...
    try {
      const userNo = user.userNo;
      const productInfo = await this.productRepository.findOne({ where:{productNo, userNo} });
      if (!productInfo) {
        throw new NotFoundException("Data Not found. : 굿즈");
      }

      if (productInfo.state !== "N1") {
        const statetInfo = await this.stateRepository.findOne({ where:{state : productInfo.state} });
        if (statetInfo) {
  ...
  async getInfo(productNo: number): Promise<any> {
    ...
    try {
      const product = await this.productRepository.findOne({ where:{productNo} });
      if (!product) {
        throw new NotFoundException("Data Not found. : 굿즈");
      }
  ...
  async getMyInfo(user: User, productNo: number): Promise<any> {
    ...
    try {
      const userNo = user.userNo;
      const product = await this.productRepository.findOne({ where:{productNo, userNo} });
      if (!product) {
        throw new NotFoundException("Data Not found. : 굿즈");
      }
      ...
      const sql1 = this.purchaseAssetRepository.createQueryBuilder('purchaseAsset')
                    .leftJoin(Asset, 
                      'asset',
                      'purchaseAsset.asset_no = asset.asset_no'
                    )
          ...
          assetList = await sql1.orderBy('purchaseAsset.purchase_asset_no', 'DESC')
                            .groupBy(`purchaseAsset.purchase_asset_no, asset.reg_name, asset.asset_name, fileAsset.file_no`)
                            .getRawMany();
   ...
   * @returns 
   */
    async getProductMyList(user: User, getProductDto: GetProductDto): Promise<any> {
