 ...
 */
  ...
  async create(createUserDto: CreateUserDto): Promise<any> {
    ...
    try {
      ...
      console.log("userWallet.vcType: "+userWallet.vcType);
      ...
      // DidWallet 저장
      const newDidWallet = new DidWallet();
      newDidWallet.userNo = userNo;
      newDidWallet.walletDid = userWallet.did;
      newDidWallet.vcType = userWallet.vcType || null;
      console.log("didWalletInfo before save: "+JSON.stringify({
        userNo: newDidWallet.userNo,
        walletDid: newDidWallet.walletDid,
        vcType: newDidWallet.vcType
      }));
      const savedDidWallet = await queryRunner.manager.save<DidWallet>(newDidWallet);
      console.log("didWalletInfo after save: "+JSON.stringify({
        userNo: savedDidWallet.userNo,
        walletDid: savedDidWallet.walletDid,
        vcType: savedDidWallet.vcType
      }));
  ...
  //     // Authledger 사용자 DID issue 요청
  //     const createDidUserDto: CreateDidUserDto = {email, nickName};
  //     const userWallet = await this.didService.createUserAsync(createDidUserDto);
  //     if (!userWallet) {      
  //       throw new NotFoundException('사용자 DID issue 요청 오류');
  //     }
  //     // // for TEST
  ...
  async createAsync(data: any): Promise<any> {
    console.log("user createAsync data : "+JSON.stringify(data));
    const userNo = data.keyValue;
    const orgId = data.orgId;
    const walletDid = data.did;
    const vcType = data.vcType;
    ...
    try {
      await queryRunner.manager.update(User, userNo, { orgId }); 

      // DidWallet 저장
      const newDidWallet = new DidWallet();
      newDidWallet.userNo = userNo;
      newDidWallet.walletDid = walletDid;
      if (vcType) {
        newDidWallet.vcType = vcType;
      }
      console.log("didWalletInfo before save: "+JSON.stringify({
        userNo: newDidWallet.userNo,
        walletDid: newDidWallet.walletDid,
      ...
      // NftWallet 저장
      let nftWalletInfo = {userNo, account: walletAddress.toLowerCase()};
      console.log("nftWalletInfo : "+JSON.stringify(nftWalletInfo));
      const newNftWallet = queryRunner.manager.create(NftWallet, nftWalletInfo);
      await queryRunner.manager.save<NftWallet>(newNftWallet);
